...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [activeView, setActiveView] = useState<string>(() => {
    if (typeof window !== 'undefined') {
      ...
      // For non-admin URLs: If logged in as Employee, ALWAYS stay in an employee view
      if (savedLoggedIn && (savedRole === 'EMPLOYEE' || savedEmployee)) {
        if (savedView && !savedView.startsWith('admin') && savedView !== 'employees') {
          return savedView;
        }
  ...
  // ADD EMPLOYEE (Admin)
  const addEmployee = (data: Partial<EmployeeProfile>) => {
    ...
    const newEmp: EmployeeProfile = {
      ...
      email: data.email || `employee${employees.length + 1}@wonderlightadventure.com`,
